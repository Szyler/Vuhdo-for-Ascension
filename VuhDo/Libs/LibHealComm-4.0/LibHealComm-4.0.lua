...
HealComm.ALL_HEALS, HealComm.CHANNEL_HEALS, HealComm.DIRECT_HEALS, HealComm.HOT_HEALS, HealComm.CASTED_HEALS, HealComm.ABSORB_SHIELDS, HealComm.ALL_DATA, HealComm.BOMB_HEALS =
	ALL_HEALS, CHANNEL_HEALS, DIRECT_HEALS, HOT_HEALS, CASTED_HEALS, ABSORB_SHIELDS, ALL_DATA, BOMB_HEALS
...
local isHealerClass = playerClass == "DRUID" or playerClass == "PRIEST" or playerClass == "SHAMAN" or
	playerClass == "PALADIN" or playerClass == "HERO"
...
HealComm.averageHealMT = HealComm.averageHealMT or {
	__index = function(tbl, index)
		...
		if (UnitLevel("level") >= MAX_PLAYER_LEVEL) then
			...
			average = average +
				(UnitLevel("player") - spellLevel) * (spellData.increase[rank] / (MAX_PLAYER_LEVEL - spellLevel))
		end
		...
		for _, pending in pairs(spells) do
			if (pending.bitType) then
				...
				if (#(tempPlayerList) > 0) then
					...
					HealComm.callbacks:Fire("HealComm_HealStopped", casterGUID, spellID, bitType, true,
...
if (playerClass == "DRUID" or playerClass == "HERO") then
	LoadClassData = function()
		...
		hotData[Rejuvenation] = {
			...
			-- coeff = 0.5,
			coeff = 0.0677,
			maxlevels = { 9, 15, 21, 27, 33, 39, 45, 51, 57, 63, 65, 68, 73, 79, 84 },
			averages = { 6, 11, 21, 33, 44, 55, 71, 88, 110, 129, 162, 168, 191, 268, 305 },
		...
		hotData[Regrowth] = {
			...
			-- coeff = 0.658,
			coeff = 0.0564,
			averages = { 11, 20, 30, 39, 49, 63, 79, 99, 122, 146, 256, 335 },
		...
		spellData[Regrowth] = {
			...
			coeff = 0.1616,
			maxlevels = { 17, 23, 29, 35, 41, 47, 53, 59, 65, 70, 76, 81 },
			levelincrease = { 1.8, 2.5, 3.1, 3.6, 4.1, 4.7, 5.3, 6, 6.8, 7.7, 10.8, 7.7 },
			averages = { avg(66, 78), avg(130, 150), avg(191, 219), avg(254, 288), avg(323, 365), avg(408, 460),
				avg(516, 579), avg(517, 580), avg(641, 716), avg(971, 1084), avg(1709, 1908), avg(2233, 2494) },
		...
		spellData[HealingTouch] = {
			-- coeff = 0.45,
			coeff = 0.47,
			maxlevels = { 5, 13, 19, 25, 31, 37, 43, 49, 55, 61, 65, 67, 73, 78, 83 },
			levelincrease = { 0.8, 1.3, 1.9, 2.7, 3, 3.4, 3.9, 4.5, 5.1, 5.7, 6.2, 6.3, 7, 9.8, 11.4 },
			levels = { 1, 8, 14, 20, 26, 32, 38, 44, 50, 56, 60, 62, 69, 74, 79 },
			averages = { avg(27, 39), avg(62, 82), avg(137, 176), avg(255, 321), avg(344, 428), avg(447, 552),
				avg(564, 691), avg(1054, 1283), avg(1142, 1383), avg(1140, 1374), avg(1368, 1649), avg(1425, 1719),
				avg(1674, 2009), avg(2835, 3418), avg(3299, 3978) }
		}
		-- Nourish
		...
		spellData[Tranquility] = {
			...
			ticks = 4,
			coeff = 0.1616,
			maxlevels = { 36, 46, 56, 66, 74, 79, 84 },
			levelincrease = { 2.2, 2.6, 3.4, 3.7, 4.5, 4.5, 4.5 },
			avg(280, 281),
			avg(411, 412),
			avg(611, 612),
			avg(877, 878),
		...
		itemSetsData["T10 Resto"] = { 50106, 50107, 50108, 50109, 50113, 51139, 51138, 51137, 51136, 51135, 51300, 51301,
			51302, 51303, 51304 }
		...
		spellData[HolyLight] = {
			...
			coeff = 0.448,
			maxlevels = { 5, 11, 19, 27, 35, 43, 51, 59, 65, 67, 74, 79, 84 },
			levelincrease = { 0.8, 1.1, 1.7, 2.4, 3.1, 3.8, 4.6, 5.2, 5.8, 6.4, 7, 10.4, 12.1 },
			averages = { avg(35, 44), avg(69, 86), avg(146, 176), avg(285, 332), avg(451, 516), avg(643, 727),
				avg(870, 983), avg(1148, 1294), avg(1464, 1650), avg(1606, 1810), avg(1938, 2195), avg(3778, 4257),
				avg(4398, 4955) },
			-- increase = {63, 81, 112, 139, 155, 159, 156, 135, 116, 115, 70, 52, 0}
		}
		-- Flash of Light
		local FlashofLight = GetSpellInfo(19750)
		spellData[FlashofLight] = {
			-- coeff = 0.265,
			levels = { 20, 26, 34, 42, 50, 58, 66, 74, 79 },
			coeff = 0.3,
			maxlevels = { 25, 31, 39, 47, 55, 63, 70, 78, 83 },
			levelincrease = { 1, 1.3, 1.6, 1.9, 2.2, 2.6, 3, 3.4, 3.9 },
			averages = { avg(69, 82), avg(106, 126), avg(162, 184), avg(220, 252), avg(294, 337), avg(384, 437),
				avg(507, 575), avg(735, 835), avg(847, 961) },
		...
		itemSetsData["T10 Holy"] = { 50865, 50866, 50867, 50868, 50869, 51270, 51271, 51272, 51273, 51274, 51165, 51166,
			51167, 51168, 51169 }

		------------------------------------------------------------------------------------------------------------------------------------
		-- Priest local variables
		------------------------------------------------------------------------------------------------------------------------------------
		...
		hotData[Renew] = {
			...
			ticks = 5,
			coeff = 0.078,
			maxlevels = { 13, 19, 25, 31, 37, 43, 49, 55, 61, 65, 69, 74, 79, 84 },
			averages = { 8, 15, 26, 35, 46, 59, 74, 94, 117, 140, 146, 160, 223, 252 },
		...
		spellData[GreaterHeal] = {
			...
			averages = { avg(36, 45), avg(56, 68), avg(107, 125), avg(225, 259), avg(342, 392), avg(452, 514),
				avg(569, 643), avg(632, 724), avg(808, 921), avg(1011, 1149), avg(1265, 1432), avg(1383, 1566),
				avg(1459, 1729), avg(1717, 2028), avg(2987, 3538), avg(3475, 4116) }
		}
		-- Prayer of Healing
		...
		spellData[PrayerofHealing] = {
			...
			averages = { avg(208, 225), avg(307, 330), avg(455, 486), avg(650, 692), avg(690, 736), avg(778, 829),
				avg(1811, 1930), avg(12499, 15000) }
		}
		-- Flash Heal
		...
		spellData[FlashHeal] = {
			...
			averages = { avg(154, 190), avg(206, 252), avg(261, 315), avg(319, 382), avg(414, 493), avg(514, 611),
				avg(649, 767), avg(730, 848), avg(880, 1023), avg(1577, 1832), avg(1886, 2193) }
		}
		-- Binding Heal
		local BindingHeal = GetSpellInfo(32546)
		spellData[BindingHeal] = {
			-- coeff = 0.215,
			levels = { 64, 72, 78 },
			coeff = 0.24,
			maxlevels = { 71, 76, 82 },
		...
		spellData[Penance] = {
			...
			levelincrease = { 4.2, 3, 3.6 },
			levels = { 60, 70, 75, 80 },
			averages = { avg(354, 400), avg(643, 727), avg(1277, 1442), avg(1483, 1676) }
		}
		-- Heal
		...
		hotData[Earthliving] = {
			...
			-- coeff = 0.40,
			coeff = 0.171,
			maxlevels = { 38, 48, 58, 68, 78, 88 },
			averages = { 16, 23, 31, 49, 64, 163 },
		...
		spellData[ChainHeal] = {
			...
			coeff = 0.356,
			maxlevels = { 45, 51, 59, 66, 73, 79, 84 },
			levelincrease = { 2.5, 2.8, 3.3, 3.8, 3.8, 3.8, 3.8 },
			averages = { avg(304, 343), avg(384, 433), avg(426, 496), avg(522, 585), avg(581, 675), avg(796, 925),
				avg(928, 1079) }
		}
		-- Healing Wave
		...
		spellData[LesserHealingWave] = {
			...
			coeff = 0.24,
			maxlevels = { 25, 33, 41, 49, 57, 65, 70, 76, 81 },
			levelincrease = { 1.7, 2.1, 2.5, 3, 3.6, 4.2, 4.2, 5, 6 },
			averages = { avg(129, 149), avg(197, 225), avg(269, 305), avg(366, 412), avg(504, 564), avg(665, 743),
				avg(830, 948), avg(1381, 1578), avg(1605, 1834) }
		...
		itemSetsData["T9 Resto"] = { 48280, 48281, 48282, 48283, 48284, 48295, 48296, 48297, 48298, 48299, 48301, 48302,
			48303, 48304, 48300, 48306, 48307, 48308, 48309, 48305, 48286, 48287, 48288, 48289, 48285, 48293, 48292,
			48291, 48290, 48294 }
		...
		local activeGraceGUID, activeGraceModifier -- Priest
		local riptideData, earthshieldList = {}, {} -- Shaman
		...
		CalculateHotHealing = function(guid, spellID)
			...
			elseif (spellName == Lifebloom) then
				...
				bombAmount = math.ceil(calculateGeneralAmount(hotData[spellName].levels[rank],
					hotData[spellName].bomb[rank], bombSpellPower, spModifier,
					healModifier + talentData[GiftofNature].current))
			...
			elseif (spellName == WildGrowth) then
				...
				for i = 1, hotData[spellName].ticks do
					table.insert(wgTicks,
						math.ceil(healModifier *
							((healAmount + tickAmount * (3 - (i - 1) * tickModifier)) + (spellPower * spModifier))))
				end
		...
		CalculateHealing = function(guid, spellName, spellRank)
			...
			elseif (spellName == HealingWave) then
				healModifier = healModifier *
					(talentData[HealingWay].spent == 3 and 1.25 or talentData[HealingWay].spent == 2 and 1.16 or talentData[HealingWay].spent == 1 and 1.08 or 1)
				healModifier = healModifier + talentData[Purification].current
				if (equippedSetCache["T7 Resto"] >= 4) then
					healModifier = healModifier * 1.05
				end
...
HealComm.healingStackMods = HealComm.healingStackMods or {
	...
	-- Tenacity
	[getName(58549)] = function(name, rank, icon, stacks)
		return icon ==
			"Interface\\Icons\\Ability_Warrior_StrengthOfArms" and stacks ^ 1.18 or 1
	end,
	-- Focused Will
...
local function parseHotHeal(casterGUID, wasUpdated, spellID, tickAmount, totalTicks, tickInterval, ...)
	...
	if (not wasUpdated) then
		HealComm.callbacks:Fire("HealComm_HealStarted", casterGUID, spellID, pending.bitType, endTime,
			unpack(tempPlayerList))
	else
		HealComm.callbacks:Fire("HealComm_HealUpdated", casterGUID, spellID, pending.bitType, endTime,
			unpack(tempPlayerList))
	end
end
...
function HealComm:CHAT_MSG_ADDON(prefix, message, channel, sender)
	...
	elseif (commType == "B" and arg1 and arg6) then
		parseHotHeal(casterGUID, false, spellID, tonumber(arg3), tonumber(extraArg), tonumber(arg5),
			string.split(",", arg6))
	...
	elseif (commType == "H" and arg1 and arg4) then
		parseHotHeal(casterGUID, false, spellID, tonumber(arg1), tonumber(extraArg), tonumber(arg3),
			string.split(",", arg4))
	...
	elseif (commtype == "U" and arg1 and arg3) then
		parseHotHeal(casterGUID, true, spellID, tonumber(arg1), tonumber(extraArg), tonumber(arg2),
			string.split(",", arg3))
	...
	elseif (commtype == "UB" and arg1 and arg5) then
		parseHotHeal(casterGUID, true, spellID, tonumber(arg3), tonumber(extraArg), tonumber(arg4),
			string.split(",", arg5))
				...
				if (data.timeout <= 0) then
					...
					elseif (data.type == "tick") then
						...
						if (pending and pending.bitType) then
							...
							HealComm.callbacks:Fire("HealComm_HealUpdated", casterGUID, pending.spellID, pending.bitType,
								endtime, unpack(data))
						end
					...
					elseif (data.type == "heal") then
						local type, amount, totalTicks, tickInterval, _, hasVariableTicks = CalculateHotHealing(data[1],
							data.spellID)
						if (type) then
							local targets, amount = GetHealTargets(type, data[1],
								hasVariableTicks and amount or math.max(amount, 0), data.spellName, data,
								hasVariableTicks)
							parseHotHeal(playerGUID, false, data.spellID, amount, totalTicks, tickInterval,
								string.split(",", targets))

							if (not hasVariableTicks) then
								sendMessage(string.format("H:%d:%d:%d::%d:%s", totalTicks, data.spellID, amount,
...
function HealComm:COMBAT_LOG_EVENT_UNFILTERED(timestamp, eventType, sourceGUID, sourceName, sourceFlags, destGUID,
	...
	elseif ((eventType == "SPELL_AURA_APPLIED" or eventType == "SPELL_AURA_REFRESH" or eventType == "SPELL_AURA_APPLIED_DOSE") and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE) then
		...
		if (hotData[spellName]) then
			...
			local type, amount, totalTicks, tickInterval, bombAmount, hasVariableTicks = CalculateHotHealing(destGUID,
				spellID)
			if (type) then
				local targets, amount = GetHealTargets(type, destGUID, hasVariableTicks and amount or math.max(amount, 0),
					spellName, hasVariableTicks)
				parseHotHeal(sourceGUID, false, spellID, amount, totalTicks, tickInterval, string.split(",", targets))

				-- Hot with a bomb!
				if (bombAmount) then
					local bombTargets, bombAmount = GetHealTargets(BOMB_HEALS, destGUID, math.max(bombAmount, 0),
						spellName)
					parseHotBomb(sourceGUID, false, spellID, bombAmount, string.split(",", bombTargets))
					sendMessage(string.format("B:%d:%d:%d:%s:%d::%d:%s", totalTicks, spellID, bombAmount, bombTargets,
						amount, tickInterval, targets))
				elseif (hasVariableTicks) then
	...
	elseif (eventType == "SPELL_AURA_REMOVED_DOSE" and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) == COMBATLOG_OBJECT_AFFILIATION_MINE) then
		...
		if (pending and pending.bitType) then
			...
			if (amount) then
				parseHotHeal(sourceGUID, true, spellID, amount, pending.totalTicks, pending.tickInterval,
				...
				if (pending.hasVariableTicks) then
					sendMessage(string.format("VU::%d:%s:%d:%s", spellID, amount, pending.totalTicks,
						compressGUID[destGUID]))
				else
					sendMessage(string.format("U:%s:%d:%d:%d:%s", spellID, amount, pending.totalTicks,
						pending.tickInterval, compressGUID[destGUID]))
				end
			end
		end
